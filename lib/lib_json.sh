...
sanitize_json() {
    ...
    if [ ! -t 0 ]; then
        # parse stream
        PATH="${AISTACK_NODEJS_BIN_PATH}:${STELLA_ORIGINAL_SYSTEM_PATH}" json5 -s 2
        return $?
    fi
    ...
    if [ ! -f "$arg" ]; then
        echo "$arg" | PATH="${AISTACK_NODEJS_BIN_PATH}:${STELLA_ORIGINAL_SYSTEM_PATH}" json5 -s 2
        return $?
    fi
    ...
    PATH="${AISTACK_NODEJS_BIN_PATH}:${STELLA_ORIGINAL_SYSTEM_PATH}" json5 -s 2 "$arg" 2>/dev/null >"$tmp_file" 
    if [ $? -ne 0 ]; then
